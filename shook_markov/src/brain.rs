...
pub struct Brain {
    name: String,
    depth: usize,
    pub chain: HashMap<Vec<Word>, Set>,
    pub head: HashSet<Word>,
}
...
impl Brain {
    ...
    pub fn generate(
        ...
        let mut words = <Vec<Word>>::with_capacity(max);
        ...
        'outer: while words.len() < min {
            ...
            while let Token::Word(word) = {
                let ctx = Self::context(words.as_slice(), self.depth);
                self.select_token(&rng, ctx)
            } {
                choose(&mut words);
                words.push(word);
                if words.len() >= max {
...
impl Adjacent {
    fn create_adjacency(&mut self, index: usize) -> bool {
        ...
        self.0.reserve(3);
        self.0.extend([index.saturating_sub(1), index, index + 1]);
        true
    }
}
